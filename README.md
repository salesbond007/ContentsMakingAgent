...
Claude(Anthropic)・OpenAIいずれかのAPIが残高不足や利用上限(429/402、"credit balance is too low"、
"insufficient_quota"等)でエラーを返した場合、`src/lib/billingError.ts`がそのエラーを検知し、
リトライしても無駄と判断して**その場で全体の処理を中断**する。中断時にSlackへ「🛑 API残高不足/利用上限の
ため中断しました」と通知し、自動実行の場合はまだ手を付けていない記事のお題を`data/resume-state.json`に
保存する。次回の自動実行(または手動実行)時に、新しくお題を探しに行く代わりにこの続きから再開し、
その日に予定していた本数を最後まで作り切る。無事に最後まで処理できたら`data/resume-state.json`は
自動的にクリアされる。手動生成が中断した場合は再開情報を保存せず、改めてリクエストし直す運用とする。
