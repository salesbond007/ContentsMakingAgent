...
コンテンツ制作パイプラインの設定(全体設定・CTA・参考記事・画像・ターゲット・自動実行の本数)をブラウザから編集し、手動での記事生成をリクエストできる管理画面。
...
- **ダッシュボード**: 概要と査読ゲートの評価軸(クリックで表示)
- **数値解析**: 公開記事数の推移
- **コンテンツ生成**: 手動生成のリクエスト(キーワード複数指定・CTA複数選択・ターゲット選択/自由記述)
- **確認待ち**: 手動生成した記事の内容(本文プレビュー・AI査読スコア/コメント)を確認し、「CMSに反映する」
  (公開)または「却下する」(破棄)を選ぶ。手動生成はAIの査読結果に関わらずここを経由しないとCMSに反映されない
- **コンテンツ設計**: 全体設定(絶対厳守ルール)・CTA・参考記事(文体参考/サービス登録)・画像(グラフ/セミナー誘致/記事サムネ)・ターゲットのプロフィール管理をまとめたグループ
- **設定**: 1日あたりの自動生成本数(0で自動生成なし)
...
4. Permissions:
   ...
   - **Variables**: Read and write(「設定」画面から1日の生成本数を変更する場合に必要。ContentsやActionsとは別の権限区分なので付け忘れに注意)
